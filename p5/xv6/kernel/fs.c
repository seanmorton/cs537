...
bmap(struct inode *ip, uint bn)
{
  ...
  uint addr, *a;
  struct buf *bp;
  if(bn < NDIRECT){ // if direct pointer
    if((addr = ip->addrs[bn]) == 0)
      ip->addrs[bn] = addr = balloc(ip->dev); // alloc if not yet
    return addr; // return that pointer
  }
  bn -= NDIRECT;

  if(bn < NINDIRECT){ // inderict pointer
    // Load indirect block, allocating if necessary.
    if((addr = ip->addrs[NDIRECT]) == 0)
      ip->addrs[NDIRECT] = addr = balloc(ip->dev); // alloc indirect block if necessary
    bp = bread(ip->dev, addr);  // read in indirect block
    a = (uint*)bp->data;
    if((addr = a[bn]) == 0){
...
// Get the correct block for a mirrored file, and get the specified copy
//
// bmap_mir(ip, 0, 0) -> 100
// bmap_mir(ip, 0, 1) -> 101
//
...
//  [1] -> 101
static uint
bmap_mir(struct inode *ip, uint bn, int copy) {
...
// if ip->type is mirrored, update fields accordingly
// hunt through code and change st-> size = ip->logical_size
// also need to set physical size correctly (sure you can do that at this point)
void
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  if (ip->type == T_MIRRORED) {
    ...
    for(tot=0; tot<n; tot+=m, off+=m, dst+=m) {
      //if (off/BSIZE > 35)
        //break; 
      bp0 = bread(ip->dev, bmap_mir(ip, off/BSIZE, 0));
      bp1 = bread(ip->dev, bmap_mir(ip, off/BSIZE, 1));
      if (memcmp(bp0->data, bp1->data, BSIZE) != 0)
        return -1;
      m = min(n - tot, BSIZE - off%BSIZE);
      memmove(dst, bp0->data + off%BSIZE, m);
      brelse(bp0);
      brelse(bp1);
    }
    ...
    for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
      bp0 = bread(ip->dev, bmap(ip, off/BSIZE));
      m = min(n - tot, BSIZE - off%BSIZE);
      memmove(dst, bp0->data + off%BSIZE, m);
      brelse(bp0);
    }
...
writei(struct inode *ip, char *src, uint off, uint n)
{
  ...
  if (ip->type == T_MIRRORED) {
    ...
    for(tot=0; tot<n; tot+=m, off+=m, src+=m){
      ...
      bp1 = bread(ip->dev, bmap_mir(ip, off/BSIZE, 1));
      m = min(n - tot, BSIZE - off%BSIZE);
      memmove(bp0->data + off%BSIZE, src, m);
